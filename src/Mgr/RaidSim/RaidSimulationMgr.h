...
private:
    ...
    // (map,difficulty) -> curated BASE creature entries (normal-mode, not difficulty_entry_N
    // variants) resolved DIRECTLY (bypasses the spawn join; captures summoned/scripted bosses with
    // 0 static spawns). Loaded from playerbots_raid_boss_loot.
